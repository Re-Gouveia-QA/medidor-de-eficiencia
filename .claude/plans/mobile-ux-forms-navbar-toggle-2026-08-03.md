...
### Phase 3: Filtros de atividades empilhados no mobile — CONCLUÍDA
...
**Resultado real:** feito em `feature/mobile-filters-stack` (branch própria, a partir de `master`).
CDP confirmou nas 3 larguras (320/360/375px): `flexDirection: column`, os 3 campos + botão "Filtrar"
com a MESMA largura entre si (largura cheia do container), sem overflow horizontal
...
(Fase 2 + Fase 4) para `master` deve ser tranquilo (git resolve automaticamente edições em regiões
diferentes do mesmo arquivo), mas confirmar depois que ambas estiverem mescladas.
